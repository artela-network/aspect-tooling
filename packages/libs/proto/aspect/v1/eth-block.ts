...
import { EthHeader } from "./eth-header";
...
export class EthBlock {
  static encode(message: EthBlock, writer: Writer): void {
    ...
    if (header !== null) {
      writer.uint32(10);
      writer.fork();
      EthHeader.encode(header, writer);
      writer.ldelim();
    }
    ...
    for (let i: i32 = 0; i < uncles.length; ++i) {
      writer.uint32(18);
      writer.fork();
      EthHeader.encode(uncles[i], writer);
      writer.ldelim();
    }
    ...
    for (let i: i32 = 0; i < transactions.length; ++i) {
      writer.uint32(26);
      writer.fork();
      AspTransaction.encode(transactions[i], writer);
      writer.ldelim();
    }
  ...
  static decode(reader: Reader, length: i32): EthBlock {
    ...
    while (reader.ptr < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.header = EthHeader.decode(reader, reader.uint32());
          break;

        case 2:
          message.uncles.push(EthHeader.decode(reader, reader.uint32()));
          break;

        case 3:
          message.transactions.push(
            AspTransaction.decode(reader, reader.uint32())
          );
          break;
  ...
  header: EthHeader | null;
  uncles: Array<EthHeader>;
  transactions: Array<AspTransaction>;
  hash: string;
  size: u64;
  dataHash: Uint8Array;

  constructor(
    header: EthHeader | null = null,
    uncles: Array<EthHeader> = [],
    transactions: Array<AspTransaction> = [],
    hash: string = "",
    size: u64 = 0,
