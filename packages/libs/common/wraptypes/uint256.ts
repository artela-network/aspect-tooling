export class Uint256 {
    ...
    public toUint8Array(): Uint8Array {
        ...
        for (let i = 0; i < 4; i++) {
            for (let j = 0; j < 8; j++) {
                result[31 - (i * 8 + j)] = <u8>(this.data[i] >> (j * 8));
            }
        }
        return result;
    }
    ...
    public fromUint8Array(bytes: Uint8Array): Uint256  {
      let value = new Uint256();
        for (let i = 0; i < 4; i++) {
            let u64value: u64 = 0;
            for (let j = 0; j < 8; j++) {
                let byteValue = <u64>(bytes[31 - (i * 8 + j)]);
                u64value |= byteValue << (j * 8);
            }
    ...
    public fromHex(hexString: string): Uint256 {
        ...
        for (let i = 0; i < 4; i++) {
            let u64value: u64 = 0;
            for (let j = 0; j < 8; j++) {
                let byteString = hexString.substring(62 - (i * 16 + j * 2), 64 - (i * 16 + j * 2));
