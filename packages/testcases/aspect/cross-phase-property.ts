...
export class CrossPhaseProperty implements IPostTxExecuteJP, IPreTxExecuteJP, ITransactionVerifier {
    ...
    preTxExecute(input: PreTxExecuteInput): void {
        const valueFromPreTxExecute = uint8ArrayToString(sys.aspect.property.get<Uint8Array>(this.propKey))
        sys.aspect.transientStorage.get<string>(this.propFromPreTxExecute).set<string>(valueFromPreTxExecute)
    }

    postTxExecute(input: PostTxExecuteInput): void {
        const valueFromPostTxExecute = uint8ArrayToString(sys.aspect.property.get<Uint8Array>(this.propKey))

        const valueFromVerifyTx = sys.aspect.transientStorage.get<string>(this.propFromVerifyTx).unwrap()
        const valueFromPreTxExecute = sys.aspect.transientStorage.get<string>(this.propFromPreTxExecute).unwrap()
        if (
